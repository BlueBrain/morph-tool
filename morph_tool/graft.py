...
from morph_tool import NoAxonException, TooManyAxonsException, NoDendriteException
...
def find_axon(axon_or_donor_axon):
    ...
    if isinstance(axon_or_donor_axon, (ImmutSection, Section)):
        if axon_or_donor_axon.type == SectionType.axon:
            ...
            return Exception('The section: {} is not an axon'.format(axon_or_donor_axon))
...
def _rotation_matrix(A, B):
    '''Returns rotation matrix that transforms 3D vector A into vector B'''
    A /= np.linalg.norm(A)
    B /= np.linalg.norm(B)
...
def grafting_position_and_direction(neuron, axon_or_donor_neuron):
    '''Find out where to put the grafted axon and the orientation it should have
    ...
    the donor axon. The returned rotation matrix is choosen in order to
    preserve the direction between the axon initial segment direction
    and all other dendrites initial segments directions

    Returns:
        a tuple (position, rotation) where 'position' is the position where to graft
        the neuron and 'rotation' is the rotation matrix to be applied to the axon
    '''
    new_axon = find_axon(axon_or_donor_neuron)
    try:
        old_axon = find_axon(neuron)
        axon_start = old_axon.points[0]
        axon_direction = _section_initial_direction(old_axon)
    ...
    rotation = _rotation_matrix(_section_initial_direction(new_axon), axon_direction)
    return axon_start, rotation
...
def graft_axon(neuron, axon_or_donor_neuron):
    ...
    position, rotation = grafting_position_and_direction(neuron, axon_or_donor_neuron)

    delete_old_axons(neuron)

    new_axon = neuron.append_root_section(donor_axon, recursive=True)

    rotate(new_axon, rotation)
